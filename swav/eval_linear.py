...
def main_worker(gpu, ngpus_per_node, logger, training_stats, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        scores = train(model, linear_classifier, optimizer, train_loader, epoch, args)
        scores_val = validate_network(val_loader, model, linear_classifier, args, best_acc)
        best_acc = scores_val[-1]
        scores_val = scores_val[:-1]
        #training_stats.update(scores + tuple(scores_val))
...
def validate_network(val_loader, model, linear_classifier, args, best_acc):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
