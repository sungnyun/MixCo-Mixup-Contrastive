...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k):
        ...
        im_q, idx1, idx2 = self.data_mixer(im_q)
        ...
        # positive logits: Nx1
        l_pos1 = torch.einsum('nc,nc->n', [q[idx1], k]).unsqueeze(-1)
        l_pos2 = torch.einsum('nc,nc->n', [q[idx2], k]).unsqueeze(-1)

        # negative logits: NxK
        l_neg1 = torch.einsum('nc,ck->nk', [q[idx1], self.queue.clone().detach()])
        l_neg2 = torch.einsum('nc,ck->nk', [q[idx2], self.queue.clone().detach()])
